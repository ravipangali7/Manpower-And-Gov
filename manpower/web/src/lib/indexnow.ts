 ...
 *    `https://www.vnvnepal.com/{key}.txt` (also place under `web/public/`).
 ...
 * 3. After a CMS publish (create/update of news, CMS pages, jobs):
 *    call Django `notify_urls` / `submit_indexnow`, or
 *    `submitIndexNow([absoluteUrl])` from this module as a client-side fallback.
 *
 * Django
 * ------
 * Production pings should use `server/core/indexnow.py` (wired to fail soft
